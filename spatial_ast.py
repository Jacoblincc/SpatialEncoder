...
class SpatialAST(_VisionTransformer):
    ...
    def extract_features(self, binaural_waveform, mask_t_prob=0.0, mask_f_prob=0.0):
        """Extract ViT patch features from an already-binaural 2-channel waveform.

        Skips the fftconvolve step in forward(). Used by DynamicSpatialAST to reuse
        the mel+IPD feature extraction pipeline without applying a separate RIR.

        Args:
            binaural_waveform: (B, 2, T) tensor – left/right channels already binaural
            mask_t_prob: time masking probability (applied only during training)
            mask_f_prob: frequency masking probability (applied only during training)

        Returns:
            x: (B, 3+N_patches, 768) – 3 CLS tokens followed by patch tokens
        """
        B, C, T = binaural_waveform.shape
        waveforms = binaural_waveform.reshape(B * C, T)
